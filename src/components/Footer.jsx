    ...
    <footer className="bg-zinc-950 border-t border-zinc-800/50 py-12">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center">
          <div className="mb-6">
            <h3 className="text-3xl font-black text-white">{footer.name}</h3>
            <p className="text-zinc-500 mt-2">
              <span className="text-zinc-300 font-semibold">{footer.title}</span> • {footer.description}
            </p>
          </div>
          <div className="flex justify-center gap-3 mb-10">
            {footer.social.map((s, i) => (
              <a key={i} href={s.href} aria-label={s.label} target="_blank" rel="noopener noreferrer"
                className="group bg-zinc-800/60 w-10 h-10 rounded-xl flex items-center justify-center border border-zinc-800 hover:border-sky-500/30 transition-all duration-300 hover:-translate-y-0.5">
                <span className="text-zinc-400 group-hover:text-sky-400 transition-colors duration-300">
